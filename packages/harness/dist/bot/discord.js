...
async function handleMessage(message) {
    ...
    if (!message.content.trim().toLowerCase().startsWith(COMMAND_PREFIX))
        return;
...
async function handleStartups(message) {
    if (!prisma) {
        await message.reply('❌ Database not connected.');
        return;
    }
...
async function handleExecute(message, args) {
    if (args.length < 1) {
    ...
    try {
        // Run the expert loop
        const result = await harness.runExpertLoop(startup.description || startup.founderBrief || '', 3);
