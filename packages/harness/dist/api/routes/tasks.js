...
export async function listTasks(req, res) {
    ...
    try {
        ...
        if (assignee)
            where.assignee = String(assignee);
        if (phase)
            where.phase = String(phase);
        if (priority)
            where.priority = String(priority);
        const tasks = await prisma.$queryRawUnsafe(`SELECT * FROM "tasks" WHERE 1=1
...
export async function getTask(req, res) {
    ...
    try {
        const { id } = req.params;
        const task = await prisma.$queryRawUnsafe(`SELECT * FROM "tasks" WHERE id = $1`, id);
...
export async function updateTask(req, res) {
    ...
    try {
        ...
        values.push(id);
        const result = await prisma.$queryRawUnsafe(`UPDATE "tasks" SET ${updates.join(', ')} WHERE id = $${paramIndex} RETURNING *`, ...values);
